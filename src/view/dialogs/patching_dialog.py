...
from ofl.fixture import Fixture, UsedFixture, make_used_fixture
...
class PatchingDialog(QtWidgets.QDialog):
    ...
    def __init__(self, fixture: tuple[Fixture, int], parent: object = None) -> None:
        ...
        self._patching_information = PatchingInformation(make_used_fixture(self._fixture, 0))
        ...
        validator = QtGui.QRegularExpressionValidator(
            QtCore.QRegularExpression(
                r"([1-9][0-9]{0,2})?(@[1-9]\d{0,2}(-([1-5]\d{1,2}|[1-9])(\/([1-3]\d{1,2}|[1-9]))?)?)?"))
