...
class BankSetCommand(Command):
    ...
    def configure_parser(self, parser: ArgumentParser) -> None:
        ...
        subparsers.add_parser("commit", exit_on_error=False, help="Commit the changes made to the bank set")
        create_parser = subparsers.add_parser(
            "create", exit_on_error=False, help="Create a new bank set and make it the selected one"
        )
        ...
        add_parser = subparsers.add_parser(
            "add", exit_on_error=False, help="Add a column to the specified bank in the selected bank set"
        )
        ...
        match args.what:
            ...
            case "create":
                if self.context.selected_bank and not self.context.selected_bank.is_linked:
                    self.context.print(
                        "ERROR: Creating a new bank set now would discard the changes made to the current selected one."
                    )
                    return False
            ...
            case "info":
                ...
                self.context.print(f"The selected bank set contains {len(self.context.selected_bank.banks)} banks.")
