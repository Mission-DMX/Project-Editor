...
class ProcessNotifier:
    ...
    def __init__(self, name: str, max_count: int) -> None:
        ...
        Args:
            name: Name of the running process.
            max_count: Initial number of maximum steps that this process needs to handle.
    ...
    def total_step_count(self, new_maximum_step_number: int) -> None:
        """
        Set the new maximum number of steps that need to be performed.

        This method triggers a Qt event processing interrupt.
        As a consequence, it will take a bit longer to return and should not be called too often.
        """
    ...
    def current_step_number(self, new_pos: int) -> None:
        """
        Set the current process indicator.

        This method will also process queued Qt events, preventing the application from freezing.
        It may take a bit longer to return though.
        """
    ...
    def current_step_description(self, new_description: str) -> None:
        ...
        A step description tells the operator what the application is currently doing.
        This method does not process Qt events and should be called before updating the current step number.
        """
...
def get_progress_changed_signal() -> SignalInstance:
    """Global process notification signal."""
...
def get_process_notifier(name: str, number_of_steps: int) -> ProcessNotifier:
    ...
    Args:
        ...
        number_of_steps: The initial step count.
...
def _close_progress_handler(p: ProcessNotifier) -> None:
    """Handle deregistration of a finished process notifier."""
