...
def _create_filter_element_for_fish(
    filter_: Filter, parent: ET.Element, for_fish: bool, om: SceneOptimizerModule
) -> None:
    """Create an XML element of type filter.
    ...
    if for_fish and filter_.is_virtual_filter:
        ...
        for output_channel_name in filter_.out_data_types:
            om.channel_override_dict[f"{filter_.filter_id}:{output_channel_name}"] = filter_.resolve_output_port_id(
                output_channel_name
            )
    else:
        if for_fish:
...
def create_channel_mappings_for_filter_set_for_fish(
    for_fish: bool, om: SceneOptimizerModule, scene_element: ET.Element
) -> None:
    """Write the channel links of the scene to the XML data.

    This method must be called *after* every filter object has been placed, as only then
    all required information is available.

    First, this method calls the `wrap_up` function from the optimizer module. After doing so,
    it queries the following information from the optimizer module:
    ...
    Args:
        for_fish: Whether this is a fish file or not.
        om: The optimizer module carrying the information.
        scene_element: The scene root element (required for the optimizer to finish operations).
                ...
                if not filter_.channel_links.get(default_val_id) or filter_.channel_links[default_val_id] == "":
                    if default_val_id == "time":
                        ...
                        _create_channel_link_element(
                            channel_link=(default_val_id, time_node + ":value"), parent=filter_element
                        )
                    else:
                        if not default_nodes.get(datatype):
                            default_nodes[datatype] = []
                        ...
                        if datatype == DataType.DT_COLOR:
                            val = "0,0,0"
                        default_value = (
                            filter_.default_values[default_val_id]
                            if filter_.default_values and default_val_id in filter_.default_values
                            else val
                        )
...
def _create_channel_link_element(channel_link: tuple[str, str], parent: ET.Element) -> ET.Element:
    """Create an XML element of a type channel link.
...
def _create_filter_configuration_element(filter_configuration: tuple[str, str], parent: ET.Element) -> ET.Element:
    """Create a XML element of type filterConfiguration.
...
def _create_initial_parameters_element(initial_parameter: tuple[str, str], parent: ET.Element) -> ET.Element:
    """Create an XML element of type initialParameters.
