...
class EffectsStack(VirtualFilter):
    ...
    def instantiate_filters(self, filter_list: list[Filter]) -> None:
        for socket in self.sockets:
            ...
            universe_filter: Filter = Filter(
                self.scene,
                f"{self.filter_id}__universeoutput__{socket_target.parent_universe}_{socket_target.start_index}",
            ...
            zero_constant_name = (
                f"{self.filter_id}__blank_color_slot_constant__"
            ...
            if socket.has_color_property:
                ...
                if color_effect is not None:
                    filter_prefix = (
                        f"{self.filter_id}__coloreffect_{socket_target.parent_universe}_{socket_target.start_index}"
                    )
                    ...
                    if socket.has_segmentation_support:
                        ...
                        if segmentation_effect:
                            filter_prefix = (
                                f"{self.filter_id}__segmentation_{socket_target.parent_universe}_"
                                f"{socket_target.start_index}"
                            )
                            segmentation_outputs = emplace_with_adapter(
                                segmentation_effect, EffectType.ENABLED_SEGMENTS, filter_list, filter_prefix
                            )

                            for i, segment_number, segment_out_port in enumerate(segmentation_outputs.items()):
                                color_index = i % len(output_dict["color"])
                                seg_split_filter_name = (
                                    f"{self.filter_id}__universeoutput__segmentsplitter_"
                                ...
                                multiply_filter_i = Filter(
                                    self.scene,
                                    seg_split_filter_name + "_multiply",
                                ...
                                combination_filter = Filter(
                                    self.scene, seg_split_filter_name + "_combine", 18, self.pos
                                )  # TODO rename
                    ...
                    for i in range(len(output_dict["color"])):
                        if color_support_of_target == ColorSupport.HAS_RGB_SUPPORT:
                            adapter_filter = Filter(
                                self.scene, color_adapter_name_base, 15, self.pos
                            )  # TODO rename type
                        elif color_support_of_target == ColorSupport.HAS_RGB_SUPPORT | ColorSupport.HAS_WHITE_SEGMENT:
                            adapter_filter = Filter(
                                self.scene, color_adapter_name_base, 16, self.pos
                            )  # TODO rename type
                        elif (
                            color_support_of_target
                            == ColorSupport.HAS_RGB_SUPPORT
                            | ColorSupport.HAS_WHITE_SEGMENT
                            | ColorSupport.HAS_AMBER_SEGMENT
                        ):
                            adapter_filter = Filter(
                                self.scene, color_adapter_name_base, 17, self.pos
                            )  # TODO rename type
                    ...
                    for segment_list in [
                        socket_target.get_segment_in_universe_by_type(FixtureChannelType.RED),
                        ...
                        for segment in segment_list:
                            fixture_chanel_name: str = socket_target.get_fixture_channel(
                                segment - socket_target.start_index
                            ).name
            ...
            if zero_constant_8bit_required:
                constant_filter = Filter(
                    self.scene, zero_constant_name + "_8bit", FilterTypeEnumeration.FILTER_CONSTANT_8BIT, self.pos
                )
                constant_filter.initial_parameters["value"] = "0"
                filter_list.append(constant_filter)
            if zero_constant_float_required:
                constant_filter = Filter(
                    self.scene, zero_constant_name + "_float", FilterTypeEnumeration.FILTER_CONSTANT_FLOAT, self.pos
                )
    ...
    def deserialize(self) -> None:
        ...
        for k, v in self._filter_configurations.items():
            ...
            if uf is None:
                logger.warning(
                    "There is no fixture associated with the address %s/%s",
                    universe,
                    channel + 1,
                )
                continue
