...
class _MsgTypeEnumTypeWrapper(google.protobuf.internal.enum_type_wrapper._EnumTypeWrapper[_MsgType.ValueType], builtins.type):
    ...
    """This message updates parameters of running filters when sent to fish, and notifies the GUI about state changes by running filters, when sent to the GUI"""
    ...
    MSGT_STATE_LIST: _MsgType.ValueType  # 20
    """If fish receives this message empty: it will reply with the same message type, filled in with all states. Otherwise this triggers an update of all specified states"""
    MSGT_EVENT_SENDER_UPDATE: _MsgType.ValueType  # 21
    """If fish receives such a message it will create or update the specified sender. If a GUI connects to fish, all configured event senders will be send to the GUI. If an event sender changed, all connected GUI clients will receive a corresponding message."""
    MSGT_EVENT: _MsgType.ValueType  # 22
    """If a GUI client sends such a message, the event will be inserted into the buffer. If an event_sender with enabled gui debugging send a new event, all connected GUI clients will receive such a message with the new event."""
    MSGT_READYMODE_UPDATE: _MsgType.ValueType  # 23
    """notification of ready mode update that should be followed by the other side."""
...
"""Console.fader_column"""
MSGT_STATE_LIST: MsgType.ValueType  # 20
"""If fish receives this message empty: it will reply with the same message type, filled in with all states. Otherwise this triggers an update of all specified states"""
MSGT_EVENT_SENDER_UPDATE: MsgType.ValueType  # 21
"""If fish receives such a message it will create or update the specified sender. If a GUI connects to fish, all configured event senders will be send to the GUI. If an event sender changed, all connected GUI clients will receive a corresponding message."""
MSGT_EVENT: MsgType.ValueType  # 22
"""If a GUI client sends such a message, the event will be inserted into the buffer. If an event_sender with enabled gui debugging send a new event, all connected GUI clients will receive such a message with the new event."""
MSGT_READYMODE_UPDATE: MsgType.ValueType  # 23
"""notification of ready mode update that should be followed by the other side."""
Global___MsgType: typing_extensions.TypeAlias = MsgType
